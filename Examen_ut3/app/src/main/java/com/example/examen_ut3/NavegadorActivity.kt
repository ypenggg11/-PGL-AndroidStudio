...
import com.example.examen_ut3.databinding.ActivityNavegadorBinding
import java.util.regex.Pattern

class NavegadorActivity : AppCompatActivity() {

    private lateinit var viewBinding: ActivityNavegadorBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        viewBinding = ActivityNavegadorBinding.inflate(layoutInflater)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.home -> {
                startActivity(Intent(this@NavegadorActivity, MainActivity::class.java))
                finish()
                return true
            }
        }
